<?class CAllFTriggerConditions{
	...
	function Add($arFields)
	{
		...
		if (!CFTriggerConditions::CheckFields('ADD', $arFields))
			return false;

		$arInsert = $DB->PrepareInsert("f_conditions", $arFields);

		$strSql =
			"INSERT INTO f_conditions(".$arInsert[0].") ".
			"VALUES(".$arInsert[1].")";
	...
	function Update($currency, $arFields)
	{
		...
		if (!CFTriggerConditions::CheckFields('UPDATE', $arFields, $currency))
			return false;

		$strCurrencyID = substr($currency, 0, 3);
		if (is_set($arFields, 'CURRENCY'))
			unset($arFields['CURRENCY']);
		$strUpdate = $DB->PrepareUpdate("f_conditions", $arFields);
		if (!empty($strUpdate))
		{
			$strSql = "UPDATE f_conditions SET ".$strUpdate." WHERE ID = '".$DB->ForSql($strCurrencyID)."' ";
			$DB->Query($strSql, false, "File: ".__FILE__."<br>Line: ".__LINE__);
	...
	function Delete($action)
	{
		...
		return $DB->Query("DELETE FROM f_conditions WHERE ID = '".$DB->ForSQL($action)."'", true);
	}
