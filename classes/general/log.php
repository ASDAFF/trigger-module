<?class CAllFTriggerLog{
	//TODO: 
	function GetTrigger($trigger)
	{
		$arRes = CAllFTriggerLog::GetByID($trigger);
		return $arRes;
	}
	...
	function Add($arFields)
	{
		...
		if (!CAllFTriggerLog::CheckFields('ADD', $arFields))
			return false;

		$arInsert = $DB->PrepareInsert("f_triggers_log", $arFields);

		//echo "<pre>";print_r($arFields);echo "</pre>";
		//echo "<pre>";print_r($arInsert);echo "</pre>";die;

		$strSql =
			"INSERT INTO f_triggers_log(".$arInsert[0].") ".
			"VALUES(".$arInsert[1].")";
	...
	function Update($currency, $arFields)
	{
		...
		if (!CAllFTriggerLog::CheckFields('UPDATE', $arFields, $currency))
			return false;
		...
		if (!empty($strUpdate))
		{
			$strSql = "UPDATE f_triggers_log SET ".$strUpdate." WHERE ID = '".$DB->ForSql($strCurrencyID)."' ";
			$DB->Query($strSql, false, "File: ".__FILE__."<br>Line: ".__LINE__);
	...
	function Delete($action)
	{
		...
		return $DB->Query("DELETE FROM f_triggers_log WHERE ID = '".$DB->ForSQL($action)."'", true);
	}
	...
	function GetList(&$by, &$order, $filter, $lang = LANGUAGE_ID)
	{
		...
		{
			$dbCurrencyList = CFTriggerLog::__GetList($by, $order, $filter, $lang);
		}
		else
		{
			$by = "sort";
