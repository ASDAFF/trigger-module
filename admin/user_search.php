...
while($arRes = $rsData->GetNext())
{
	...
	$row->AddViewField("NAME", $arRes["LOGIN"]."<input type=hidden name='n".$arRes["ID"]."' id='name_".$arRes["ID"]."' value='".CUtil::JSEscape(htmlspecialcharsbx($arRes["LOGIN"]))."'>");
	...
	$row->AddActions(array(
		array(
			...
			"ACTION"=>"javascript:SelEl('".CUtil::JSEscape($get_xml_id? $arRes["XML_ID"]: $arRes["ID"])."', '".CUtil::JSEscape($arRes["LOGIN"])."')",
		),
	));
}
