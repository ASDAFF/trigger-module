...
if($CURRENCY_RIGHT=="W" && $arID = $lAdmin->GroupAction())
{
	...
	{
		$rsData = CFTriggerConditions::GetList($by, $order);
	...
	foreach($arID as $ID)
	{
		...
		case "delete":
			if ($CURRENCY_RIGHT=="W")
				if (!CFTriggerConditions::Delete($ID))
				{
					if ($ex = $APPLICATION->GetException())
...
{
	$row =& $lAdmin->AddRow($f_ID, $arRes, "/bitrix/admin/condition_edit.php?ID=".$f_ID."&lang=".LANGUAGE_ID, GetMessage('ACTION_A_EDIT'));
	...
	$arActions[] = array(
		...
		"ACTION"=>$lAdmin->ActionRedirect("/bitrix/admin/condition_edit.php?ID=".$f_ID."&lang=".LANGUAGE_ID)
	);
...
$aContext = array(
	array(
		...
		"LINK"=>"/bitrix/admin/condition_edit.php?lang=".LANGUAGE_ID,
